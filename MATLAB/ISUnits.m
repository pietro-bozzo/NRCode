...
args = {regs,'sleepm','shuffle',1000,'load',true,'save',false,'verbose',true};
stat = runBatch(batch_file,@USUnitModulation_,args);
clearvars args
...
n_bins = 50;
args = {regs,'sleepm','n_bins',n_bins,'shuffle',1,'load',false};
[~,distr,shuffled,session] = runBatchParallel(batch_file,@USUnitModulation_,args);

[~,~,session] = unique(vertcat(session{:}),'stable');
animal = reverseCellStruct(stat,@(x) numel(x.h(~isnan(x.h))),'fields',regs);
...
for i = 1 : 4
  ...
  PlotColorMap(sorted_distr,'cutoffs',[0,1.6],'x',phase_bins,'bar',labels(i),'map','parula','ax',axs(i))
  x_lim = xlim;
...
ylabel(axs(1),'units'), set(axs(end),'Color','none','XColor','none','YColor','none'), set(axs,'YTick',[1000,2000])
% set colorbar width
children = gcf().Children().Children(); % get handle to colorbar axis
children(1).Position = children(1).Position .* [1.02,1,0.5,1];
clearvars labels ns_ind sort_ns sig_ind sort_sig sort_ind sorted_distr axs i x_lim children
saveFlag && saveFig(gcf,'ReuSlowRythm/Results/FinalFigures/ISUnits/3a_colormap',["png","svg"]);

%% S3a) example of entrainment assessment
i = 1;
sig_ind = find(h.(regs(i)));
[~,axs] = makeFigure('example',"Example "+upper(regs(i))+" unit",[1,2],'size',[7,3],'polar',[false,true]);

% linear
x = [phase_bins,2*pi+phase_bins];
y = circularSmooth(distr.(regs(i))(sig_ind(17),:),'gaussian',5);
...
polarplot(axs(2),phase_bins-pi/2,circularSmooth(distr.(regs(i))(sig_ind(17),:),'gaussian',5),'Color',paperColors(color_ind(i)))
...
[~,axs] = makeFigure('subpop','Subpopulations of entrained units',[1,4],'size',[1700,400]);
OnOffAxes(2600,axs)
phase_shift = 3/4;
...
%% c) status
[~,axs] = makeFigure('psi','Unit preferred status',[1,4],'size',[1400,400]);
...
for i = 1 : 4
  ...
  set(axs(i),'YLim',[0,3],'YTick',[0,1,2])

  % add inset with subpopulation percentages
  ax = makeInset(0.6,0.6,0.38,0.38,'ax',axs(i));
...
ylabel(axs(2:end),'')
saveFlag && saveFig(gcf,'ReuSlowRythm/Results/FinalFigures/ISUnits/3c_status',["png","svg"]);
